 ...
 */
public class UndoChart {

  private DefaultCategoryDataset undoDS;
  private NumberAxis undoSizeAxis;
...
//      ExecuteDisplay.addWindowHolder(myChartPanel, "Undo - " + instanceName);
//    }
//    else {
      ChartFrame undoCF = new ChartFrame(ConsoleWindow.getInstanceName() + ": Undo Chart", myChart, true);
...
//    }
  ...
  private JFreeChart makeChart(QueryResult myResult) {
    undoDS = new DefaultCategoryDataset();

    // create the chart 
    myChart = ChartFactory.createBarChart("", "Sid", "Undo in Kb", undoDS,PlotOrientation.VERTICAL, true, true, false);
...
//    myChart.setTitle("Undo (top " + offSet + ")");
    myChart.getTitle().setFont(new Font("SansSerif", Font.PLAIN, Properties.getAxisLabelFontSize()));

    // configure the events axis 
    undoSizeAxis = (NumberAxis)myPlot.getRangeAxis();
    undoSizeAxis.setLowerBound(0);
    undoSizeAxis.setAutoRange(true);
    TickUnitSource integerTicks = NumberAxis.createIntegerTickUnits();
    undoSizeAxis.setStandardTickUnits(integerTicks);
    undoSizeAxis.setLabelFont(new Font("SansSerif", Font.PLAIN, Properties.getAxisLabelFontSize()));
    undoSizeAxis.setTickLabelFont(new Font("SansSerif", Font.PLAIN, Properties.getAxisLabelTickFontSize()));
  ...
  public void updateChart(QueryResult myResult) {     
    ...
    for (int i = 0; i < max; i++) {
      undoDS.addValue(Double.valueOf(resultSet[i][1]).doubleValue(), "undo in kb", resultSet[i][0]);
    }
    
    CategoryPlot myPlot = myChart.getCategoryPlot();
    myPlot.setDataset(undoDS);
  }
