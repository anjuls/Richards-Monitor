/*
 * SPPReloadInvalidationsB.java        13.14 07/06/05
 *
 ...
 * Change History
 * ==============
 * 14/03/06 Richard Wright Corrected spelling of second y axis.
 ...
 * 05/03/12 Richrad Wright Modified to use awrCache
 */
 ...
 *
 */
public class SPReloadInvalidationsB extends JButton {
   ...
   * @param statusBar
   */
  public SPReloadInvalidationsB(StatspackAWRInstancePanel statspackPanel, String buttonName, JScrollPane scrollP, JLabel statusBar) {
    super(buttonName);
...
//      statspackPanel.clearScrollP();
      ChartPanel myChartPanel = createChart();
...
//      scrollP.getViewport().add(myChartPanel, null);
  ...
  public ChartPanel createChart() throws Exception {
    ...
    while (i < snapIdRange.length - 1) {
      ...
      if (Properties.isAvoid_awr() == false && ConsoleWindow.getDBVersion() >= 
        10) {
        cursorId = "stats$ReloadInvalidationsAWR.sql";
      }
      else {
        cursorId = "stats$ReloadInvalidations.sql";
      }
